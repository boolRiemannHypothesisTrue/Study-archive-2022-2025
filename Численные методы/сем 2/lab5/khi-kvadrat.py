...
def midpoint_rule(f, a, b, h):
    ...
    return integral * h
'''
def rect_sum_left(f, left_border, step):  
   sum=0
   seg=int(1/step)  #  сег это число разбиений отрезка интегрирования 
   for i in range(seg):sum+=step*f(left_border+i*step)
   return sum
...
def rect_sum_cental(f, left_border, step):  # // левые прямоугольники от меня
   sum=0
   seg=int(1/step)  # сег это число разбиений отрезка интегрирования 
   for i in range(seg):sum+=f(left_border+i*step + step/2)
   sum=step*sum
   return sum
...
for i in range(15):
    ...
    #Sk= midpoint_rule(p, N, N+1, step_arr[i])
