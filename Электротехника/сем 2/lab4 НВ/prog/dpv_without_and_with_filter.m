...
R = 100000;          % Load resistance (Ohms)
C = 1000e-6;     % Capacitance of the filter (Farads)
...
% Rectified output voltage (on the load) - Full-wave rectified
uload = abs(u2);  % Take absolute value for full-wave rectification
...
% Rectified output voltage uload plot (non-filtered, full-wave rectification)
subplot(3, 1, 2);
plot(t, uload, 'k', 'LineWidth', 1.5);  % 'b' stands for blue color
title('$u_{load}(t)$: Output voltage (non-filtered, full-wave)', 'Interpreter', 'latex', 'FontSize', 12);
xlabel('Time $t$ (s)', 'Interpreter', 'latex', 'FontSize', 12);
...
title('$u_{load}(t)$: Output voltage with C-filter (full-wave)', 'Interpreter', 'latex', 'FontSize', 12);
xlabel('Time $t$ (s)', 'Interpreter', 'latex', 'FontSize', 12);
ylabel('$u_{load}(t)$ (V)', 'Interpreter', 'latex', 'FontSize', 12);
grid on;
grid minor;
